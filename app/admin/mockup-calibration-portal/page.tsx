...
function spreadPlacementAcrossColors(
  ...
  sourceColor: MockupColor
) {
  const next = deepClone(map);
  const product = next[productType];
  if (!product) return next;
  const source = product[sourceColor] || product.white;
  if (!source) return next;

  for (const colorKey of Object.keys(product) as MockupColor[]) {
    const entry = product[colorKey];
    if (!entry) continue;
    entry.placement = deepClone(source.placement);
    entry.opacity = source.opacity;
    entry.dropShadow = source.dropShadow;
  }
  return next;
}

export default function MockupCalibrationPortalPage() {
  ...
  const saveButtonLabel = isApparel ? "Save for all colours" : "Save placement";
  ...
  const saveDefaults = async () => {
    setSaveMessage(null);
    const outgoing = isApparel ? spreadPlacementAcrossColors(map, productType, color) : map;
    ...
    const res = await fetch("/api/admin/mockup-placements", {
      ...
      body: JSON.stringify({ placements: outgoing }),
    });
    ...
    setSaveMessage(
      isApparel
        ? `Saved ${productType} placement for all colours`
        : `Saved ${productType} placement`
    );
  };
  ...
  if (!enabled) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-10">
        <h1 className="text-3xl font-black">Mockup Calibration Portal</h1>
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <p className="mt-2 text-black/70">
        Tee and hoodie save buttons apply to all colours. Mug and card save only the current placement.
      </p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-12">
        <section className="space-y-4 rounded-2xl border border-black/10 bg-white p-4 lg:col-span-4">
          ...
          <button onClick={saveDefaults} className="w-full rounded-xl bg-black px-3 py-2 text-sm font-bold text-white">
            {saveButtonLabel}
          </button>
